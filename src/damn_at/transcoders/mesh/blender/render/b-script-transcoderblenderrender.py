        ...
        if len(materials) == 0:
            ...
            if mesh.uv_textures.active and mesh.uv_textures.active.data:
                ...
                for f, uv in zip(mesh.tessfaces, mesh.uv_textures.active.data):
                    # TODO:
                    # Ask for the size first else
                    # has_data returns false for no reason...
                    if uv.image and uv.image.size[0] and uv.image.has_data:
                        # print("I: mesh.active_uv_texture: uv.image", m1i,
                        #      materials[m1i].name)
                        f.material_index = m1i
                    else:
                        # print("I: mesh.active_uv_texture: no data", m2i,
                        #      materials[m2i].name, uv.image.size[0])
...
def bounding_box_for_empty(empty):
    ...
    for obj in empty.dupli_group.objects:
        bound_box = [x for x in [obj.matrix_world*Vector(v)
