...
class Analyzer(object):
        ...
        for mimetype, analyzer in self.analyzers.items():
            try:
                module = imp.load_source(
                    'damn_at.metadata.' + (mimetype.replace('.', '__')),
                    os.path.join(os.path.dirname(analyzer.path), 'metadata.py')
                )
    ...
    def _file_metadata(self, an_uri, file_descr):
        ...
        file_descr.metadata['st_size'] = MetaDataValue(
        ...
        file_descr.metadata['st_ctime'] = MetaDataValue(
            type=MetaDataType.STRING,
            string_value=time.asctime(time.localtime(stat.st_ctime))
        )
        file_descr.metadata['st_mtime'] = MetaDataValue(
            type=MetaDataType.STRING,
            string_value=time.asctime(time.localtime(stat.st_mtime))
        )
        ...
        try:
            ...
            logger.warn("Unable to extract repository information:"
                        " %s", str(repo_exception))
    ...
    def analyze_file(self, an_uri):
        ...
        if not is_existing_file(an_uri):
            raise AnalyzerFileException(
                'E: Analyzer: No such file "%s"!' % (an_uri)
            )
        ...
        if mimetype in self.analyzers:
            try:
                ...
                import traceback
                traceback.print_exc()
                raise AnalyzerException("E: Failed to analyze %s because of "
                                        "%s" % (an_uri, str(ex)))
        else:
            raise AnalyzerUnknownTypeException(
                "E: Analyzer: No analyzer for %s (file: %s)" % (mimetype,
                                                                an_uri)
            )
...
def analyze(analyzer, metadatastore, file_name, output, forcereanalyze=False):
    """
    TODO: move hashing to generic function and metadatastore usage to the
    ...
    if descr.assets:
        ...
        for asset in descr.assets:
            output.info('  -->%s  (%s)' % (asset.asset.subname,
                                           asset.asset.mimetype))
            if asset.metadata:
                for key, val in asset.metadata.items():
                    asset_type, val = get_metadatavalue_type(val)
                    output.info('    - %s  %s (%s)' % (key, val, asset_type))
...
def main():
    ...
    parser.add_argument(
        'path',
        help='path to a file or a directory to analyze recursively'
    )
    parser.add_argument(
        '--force',
        action='store_true',
    ...
    if os.path.isfile(args.path):
        analyze(analyzer, metadatastore, os.path.abspath(args.path), output,
